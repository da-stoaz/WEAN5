            ...
            <div>
                <nav class="flex text-sm text-gray-500 mb-1" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 md:space-x-2 ">
                        ...
                        {{-- 2. ALL links are generated by the child view --}}
                        @hasSection('breadcrumbs')
                        <li><span class="mx-1 text-gray-400">/</span></li>
                        @yield('breadcrumbs')
                        @endif
                        
                        {{-- 3. REMOVED the automatic final breadcrumb items from here --}}
                        {{-- The child view must now include the final <li> item itself --}}
                        
                    </ol>
                </nav>

                <h1 class="text-2xl font-bold text-gray-900 tracking-tight">
                    {{-- USE DEDICATED SECTION for H1 --}}
                    @yield('h1_title')
                </h1>
            </div>
